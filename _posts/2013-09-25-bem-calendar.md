...
In javascript ```i-bem``` provides methods like ```findBlockInside```/```findBlockOn``` for finding nested blocks. It also provides methods ```findBlockOutside```/```findBlocksOutside``` which conflicts with the idea of independent blocks. I do not recommend to use this method in your project. Just [disable them](https://github.com/tarmolov/bem-aib).
